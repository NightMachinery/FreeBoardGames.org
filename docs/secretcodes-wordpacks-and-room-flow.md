...
Secret Codes built-in wordpacks live in `web/src/games/secretcodes/wordpacks/` as `.txt` files. Each non-empty line is one card word. Leading/trailing whitespace is ignored, and lines starting with `#` are comments. New `.txt` files in this directory are auto-discovered at web build time and shown as wordpack options using the filename without the `.txt` extension as the display label.
