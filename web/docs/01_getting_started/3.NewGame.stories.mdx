...
Then, `git clone` your new forked repo, and run `pnpm install` to install all dependencies. After everything is downloaded, run `pnpm run dev` to run FBG on your machine locally, it should be available at [http://localhost:3000](http://localhost:3000/).
...
Done! You can run now `pnpm run dev <<GAME CODE>>` and you should be able to see your new game on the home page! It will have two game modes, one for playing with friends locally and another for playing with friends over the internet. Because we have `debug: true` in `config.ts`, you will be able to see the boardgame.io debug menu.
